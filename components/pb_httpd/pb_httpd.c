    ...
    const pb_policy_snapshot_t *snap)
{
    ...
    const char *operation = "event header send";
    errno = 0;
    esp_err_t r = httpd_resp_send_chunk(req, head, n);
    if (r == ESP_OK) {
        operation = "data prefix send";
        errno = 0;
        r = httpd_resp_send_chunk(req, "data: ", 6);
    }
    if (r == ESP_OK) {
        operation = "JSON payload send";
        errno = 0;
        r = httpd_resp_send_chunk(req, json, strlen(json));
    }
    if (r == ESP_OK) {
        operation = "event terminator send";
        errno = 0;
    ...
    cJSON_free(json);
    if (r != ESP_OK)
        sse_log_failure(connection_id, socket_fd, operation, r, socket_errno);
    return r;
}
