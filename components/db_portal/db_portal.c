...
static const char DIAG_BODY[] =
    ...
    "var traceStart=(window.performance&&performance.now)?performance.now():Date.now(),traceRequest=0,pollSeq=0,pollFailures=0,firstSseEvent=false,firstDom=false,firstOne=false,firstTwo=false;"
    "function traceClock(){return(window.performance&&performance.now)?performance.now():Date.now();}"
    "function trace(event,meta){var line='DIAGTRACE +'+Math.round(traceClock()-traceStart)+'ms '+event;if(meta)line+=' '+meta;line+=' visible='+document.visibilityState+' focus='+document.hasFocus();console.log(line);}"
    "function traceError(e){return'name='+encodeURIComponent((e&&e.name)||'unknown')+' message='+encodeURIComponent((e&&e.message)||String(e));}"
    "function tracePending(kind,id){var timers=[5000,15000,30000].map(function(ms){return setTimeout(function(){trace(kind+'_pending','id='+id+' age_ms='+ms);},ms);});return function(){timers.forEach(function(timer){clearTimeout(timer);});};}"
    "trace('init','url='+encodeURIComponent(location.href));"
    "document.addEventListener('visibilitychange',function(){trace('visibilitychange','state='+document.visibilityState);});"
    "window.addEventListener('focus',function(){trace('focus');});"
    "window.addEventListener('blur',function(){trace('blur');});"
    ...
    "function apply(s,source){source=source||'unknown';trace('apply_start','source='+source);try{"
    "if(!s||s.api_version!==2){trace('apply_skip','source='+source+' api_version='+(s&&s.api_version));return;}"
    ...
    "if(!firstDom&&chv!=null){firstDom=true;trace('first_dom_update','source='+source+' chamber='+chv+' elapsed_ms='+Math.round(traceClock()-traceStart));}"
    ...
    "$('d-ctl').textContent=(tc.controller||'--')+' \u00b7 preferred '+(tc.preferred_source||'--');"
    "$('d-pv').textContent=f1(tc.process_variable_c)+' \u00b0C ('+(tc.effective_source||'--')+')';"
    "$('d-demand').textContent=tc.controller_output==null?'--':(Number(tc.controller_output)*100).toFixed(0)+'%';"
    ...
    "samples.push({t:now-t0,ch:chv,pt:ptv,ps:pt.status,o:out?1:0,m:s.mode,fl:fl?1:0,rs:saf.reason||'',pk:peak,ctl:tc.controller||'',pref:tc.preferred_source||'',eff:tc.effective_source||'',pv:tc.process_variable_c,co:tc.controller_output,ro:tc.requested_output,cap:tc.approach_cap,alim:tc.approach_limited?1:0,lf:tc.local_foldback_active?1:0,efb:tc.element_foldback_active?1:0,oc:tc.output_constrained?1:0,con:tc.constraint||''});"
    "if(samples.length>MAX)samples.shift();"
    "if(samples.length===1&&!firstOne){firstOne=true;trace('sample_count','count=1 source='+source);}"
    "if(samples.length===2&&!firstTwo){firstTwo=true;trace('sample_count','count=2 source='+source);}"
    "$('d-stat').textContent=samples.length+' samples';draw();"
    "trace('apply_complete','source='+source+' samples='+samples.length);"
    "}catch(e){trace('apply_error','source='+source+' '+traceError(e));throw e;}}"
    "function draw(){"
    ...
    "var r=['t_s,chamber_c,ptc_c,ptc_status,out,mode,fault,reason,peak_c,controller,preferred_source,effective_source,process_variable_c,controller_output,requested_output,approach_cap,approach_limited,local_foldback,element_foldback,output_constrained,constraint'];"
    "samples.forEach(function(s){r.push([s.t.toFixed(1),(s.ch==null?'':s.ch.toFixed(1)),(s.pt==null?'':s.pt.toFixed(1)),s.ps,s.o,s.m,s.fl,'\"'+String(s.rs).replace(/\"/g,'\"\"')+'\"',s.pk.toFixed(1),s.ctl,s.pref,s.eff,(s.pv==null?'':Number(s.pv).toFixed(1)),(s.co==null?'':Number(s.co).toFixed(4)),(s.ro==null?'':Number(s.ro).toFixed(4)),(s.cap==null?'':Number(s.cap).toFixed(4)),s.alim,s.lf,s.efb,s.oc,s.con].join(','));});"
    ...
    "function poll(){if(polling){trace('poll_skip','reason=in_flight');return;}polling=true;var seq=++pollSeq,id=++traceRequest,outcome='pending';"
    "trace('poll_start','seq='+seq+' id='+id+' failures='+pollFailures+' delay_ms='+pollDelay);var clearPending=tracePending('poll',id);"
    "fetch('/api/v2/state',{cache:'no-store'})"
    ".then(function(r){trace('poll_response','seq='+seq+' id='+id+' status='+r.status+' ok='+r.ok);if(!r.ok)throw new Error('http_'+r.status);trace('poll_json_start','seq='+seq+' id='+id);return r.json().then(function(s){trace('poll_json_success','seq='+seq+' id='+id);return s;},function(e){trace('poll_json_failure','seq='+seq+' id='+id+' '+traceError(e));throw e;});})"
    ".then(function(s){trace('poll_apply','seq='+seq+' id='+id);apply(s,'poll');outcome='success';pollFailures=0;pollDelay=2000;conn('Telemetry: polling (live stream unavailable)',true);})"
    ".catch(function(e){outcome='failure';pollFailures++;trace('poll_failure','seq='+seq+' id='+id+' failures='+pollFailures+' '+traceError(e));conn('Telemetry unavailable \u2014 retrying',true);pollDelay=Math.min(10000,Math.round(pollDelay*1.6));})"
    ".finally(function(){clearPending();polling=false;var nextDelay=pollDelay+Math.random()*250;trace('poll_complete','seq='+seq+' id='+id+' outcome='+outcome);trace('poll_scheduled','seq='+(seq+1)+' delay_ms='+Math.round(nextDelay)+' failures='+pollFailures);setTimeout(poll,nextDelay);});}"
    "function events(){trace('sse_construct','url=/api/v2/events');var es=new EventSource('/api/v2/events');"
    "es.onopen=function(){trace('sse_open','ready_state='+es.readyState);};"
    "function ev(e){if(!firstSseEvent){firstSseEvent=true;trace('sse_first_event','type='+e.type);}try{trace('sse_apply','type='+e.type);apply(JSON.parse(e.data),'sse');conn('Telemetry: live stream',false);}catch(_){}}"
    "es.addEventListener('state',ev);es.addEventListener('telemetry',ev);"
    "es.onerror=function(){trace('sse_error','ready_state='+es.readyState);es.close();trace('sse_closed','ready_state='+es.readyState);conn('Live stream unavailable \u2014 switching to polling',true);trace('poll_transition','reason=sse_error');poll();};}"
    "var initialId=++traceRequest,initialOutcome='pending';trace('initial_fetch_start','id='+initialId);var clearInitialPending=tracePending('initial_fetch',initialId);"
    "fetch('/api/v2/state',{cache:'no-store'}).then(function(r){trace('initial_response','id='+initialId+' status='+r.status+' ok='+r.ok+' content_type='+encodeURIComponent(r.headers.get('content-type')||''));if(!r.ok)throw new Error('http_'+r.status);trace('initial_json_start','id='+initialId);return r.json().then(function(s){trace('initial_json_success','id='+initialId);return s;},function(e){trace('initial_json_failure','id='+initialId+' '+traceError(e));throw e;});})"
    ".then(function(s){trace('initial_apply','id='+initialId);apply(s,'initial');initialOutcome='success';})"
    ".catch(function(e){initialOutcome='failure';trace('initial_fetch_failure','id='+initialId+' '+traceError(e));})"
    ".finally(function(){clearInitialPending();trace('initial_fetch_complete','id='+initialId+' outcome='+initialOutcome);});events();"
    "})();</script></body></html>";
