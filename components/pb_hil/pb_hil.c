...
static cJSON *state_json(void)
{
    ...
    uint32_t zc_min_interval_us = 0;
    uint32_t zc_rejected_count = 0;
    pb_fan_zc_diag(&zc_count, &zc_interval_us, &zc_min_interval_us,
                   &zc_rejected_count);
    ...
    cJSON_AddNumberToObject(io, "zero_cross_min_interval_us", zc_min_interval_us);
