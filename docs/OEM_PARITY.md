...
| Fan-only filtration mode | **Implemented (one intentional divergence)** | Stock runs the blower alone as a **standing** low band — the fan comes on once the bed **setpoint** reaches `filter_temp`, before the heater engages, with no mode/arming step. DragonBreath matches that shape: the fan-only band is a standing behavior driven by the bed setpoint, independent of DragonBreath's mode (it filters even while idle), plus a separate mode-independent manual filtration fan (dashboard toggle + `filter` API command). **Divergence:** the automatic band ships **off by default (opt-in via Settings → Filtration: Auto)**, where stock leaves it **on by default** — a deliberate choice so an update never starts running a user's fan unprompted; users who want stock behavior enable it once. `filter_temp` default 30 °C. Fan-only in both cases — the heater is never engaged by filtration. |
